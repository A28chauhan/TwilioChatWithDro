...
public class LoginViewModel extends ViewModel {
    ...
    public void login(User user) {
        ...
        if(result != null && !result.equals("")){
            if (result instanceof Result.Success) {
                LoggedInUser data = ((Result.Success<LoggedInUser>) result).getData();
                loginResult.setValue(new LoginResult(new LoggedInUserView(data.getResultResponse())));
            } else {
                LoggedInUser data = ((Result.Failure<LoggedInUser>) result).getData();
                loginResult.setValue(new LoginResult(data.getError()));
            }
        }
    ...
    public boolean loginDataChanged(String username) {
        if (!isUserNameValid(username)) {
            return false;
        } else {
            return true;
